...
var bothObjects = function(a, b) {
  return typeof a === 'object' && typeof b === 'object'
}

var keysAreEqual = function(keyA, keyB) {
  return bothObjects(keyA, keyB) ?
    isEqual(keyA, keyB) :
    keyA === keyB
}

var isEqual = function(objectA, objectB) {
  let propsA = Object.getOwnPropertyNames(objectA)
  let propsB = Object.getOwnPropertyNames(objectB)

  if (propsA.length !== propsB.length) {
    return false
  }

  return propsA.every(prop => keysAreEqual(objectA[prop], objectB[prop]))
}
...
var where = function(collection, props) {
  return collection.map(item => {
    for (var attribute in props) {
      ...
      if (!keysAreEqual(item[attribute], value)) {
        return false
      }
    }
