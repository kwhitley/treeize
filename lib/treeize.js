...
Treeize.prototype.signature = function(row, auto) {
  ...
  _.each(row, function(value, key) {
    ...
    attr.split      = attr.fullPath.split(this.options().input.delimiter);
    attr.path       = _.initial(attr.split).join(opt.input.delimiter).replace(/[\*\+\-]/gi,'');
    attr.parent     = _.initial(attr.split, 2).join(opt.input.delimiter).replace(/[\*\+\-]/gi,'');
...
Treeize.prototype.setSignature = function(row, auto) {
  return this.signature(row, auto);
};

Treeize.prototype.clearSignature = function() {
  this.data.signature = { nodes: [] };
...
Treeize.prototype.grow = function(data, options) {
  ...
  if (!signature.nodes.length) {
    console.log('setting signature from first row of data (auto)');
    ...
    if (_.isArray(data[0])) {
      // remove first row from flat array after reading it for signature
      data.shift(); //TODO: BUG: This removes row from from original data source, clone all data before use within grow
    }
  }
  ...
  _.each(data, function(row) {
    ...
    if (!opt.input.uniformRows) {
      console.log('setting signature from new row of data (auto)');
    ...
    _.each(this.signature().nodes, function(node) {
      ...
      if (!opt.output.prune || !_.isEmpty(blueprintExtended)) {
        // ROOT CASE
        if ((opt.output.resultsAsObject && node.depth === 0) || !(trail = trails[node.parent])) {
          this.log('PARENT TRAIL NOT FOUND (ROOT?)');
          if (opt.output.resultsAsObject) {
            _.extend(trail = root, blueprintExtended);
...
Treeize.prototype.options = function(options) {
  ...
  this.log('OPTIONS (fn)>>', this._options);
  return this;
};
