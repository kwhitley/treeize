...
Treeize.prototype.grow = function(data, options) {
  ...
  if (!signature.nodes.length) {
    ...
    // remove header row in flat array data
    if (_.isArray(data[0])) {
      data.shift();
    }
  ...
  // clear signature between growth sets - TODO: consider leaving this wipe pass off if processing multiple identical sources (add option)
  this.signature([]);
