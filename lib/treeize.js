...
Treeize.prototype.grow = function(data, options) {
  ...
  if (opt.data.resultsAsObject && _.isArray(this.data.tree)) {
    this.data.tree = {};
  }
  ...
  _.each(data, function(row) {
    ...
    if (opt.data.resultsAsObject) {
      trails[''] = trail;
    }

    if (!opt.data.uniformRows) {
    ...
    _.each(this.signature().nodes, function(node) {
      ...
      if (!opt.data.prune || !_.isEmpty(blueprintExtended)) {
        // ROOT CASE
        if ((opt.data.resultsAsObject && node.depth === 0) || !(trail = trails[node.parent])) {
