...
Treeize.prototype.signature = function(row, auto) {
  ...
  _.each(row, function(value, key) {
    ...
    if (collectionFlag) {
      ...
      node.flags = true;
      node.isCollection = attr.node.match(/^\+|\+$/g).length;
...
Treeize.prototype.grow = function(data, options) {
  ...
  if (!signature.nodes.length) {
    ...
    signature = this.setSignature(data[0], true).getSignature();
  ...
  this.log('SIGNATURE>', util.inspect(this.signature(), false, null));
  ...
  _.each(data, function(row) {
    ...
    _.each(this.signature().nodes, function(node) {
      ...
      if (!opt.output.prune || !_.isEmpty(blueprintExtended)) {
        ...
        //   // extend root object when resultsAsObject enabled
        //   _.extend(trail = root, blueprintExtended);
        //   this.log('extending blueprint onto root>', trail);

            // if (node.name && trail[node.name] === undefined) {
